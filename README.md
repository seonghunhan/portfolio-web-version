...
7. Approved : 승인됨
8. Denied : 거부됨
9. 고객이 대출 상품을 신청하면 Pending 상태로 뜨게 되고, 관리자가 고객의 정보를 고려하여 신청을 승인해줄 지 거부할 지 제어할 수 있다.
10. Approved 상태가 되면 승인된 날짜부터 대출이 진행되어 상환날짜와 상환 금액, 상환 상태가 업데이트 된다.
11. Denied 상태가 되면 위와 같은 정보가 표시되지 않는다.
12. 검색기능이 있어 같은 사용자가 같은 대출 상품을 연속으로 신청한 경우 쉽게 확인하여 처리할 수 있다.
